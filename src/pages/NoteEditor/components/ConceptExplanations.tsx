  ...
  useEffect(() => {
    ...
    const updateConceptPositions = () => {
      ...
      document.querySelectorAll('.concept-mark').forEach(element => {
        ...
        const mode = element.getAttribute('data-concept-mode');
        
        // Only track positions for fast mode concepts
        if (tag && mode === 'fast') {
    ...
    const handleConceptClick = (e: MouseEvent) => {
      ...
      if (conceptMark) {
        ...
        const mode = conceptMark.getAttribute('data-concept-mode');
        
        if (tag) {
          const concept = concepts.find(c => c.tag === tag);
          if (!concept) return;

          if (mode === 'normal' || mode === 'pro') {
            // Call the onConceptClick handler for normal/pro concepts
            if (onConceptClick) {
              onConceptClick(concept);
            }
          } else if (mode === 'fast') {
            // Toggle inline explanation for fast mode
