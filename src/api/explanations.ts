...
export const explanationsAPI = {
  getExplanations: async (userId: string): Promise<ExplanationsResponse> => {
    try {
      const response = await fetch('https://backend-aec-experimental.onrender.com/get_generated_explanations', {
        method: 'POST',
        headers: {
  ...
  deleteExplanation: async (userId: string, conversationId: string): Promise<DeleteResponse> => {
    try {
      const response = await fetch('https://backend-aec-experimental.onrender.com/delete_conversation', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
