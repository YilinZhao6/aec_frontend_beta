...
export const notesEditorAPI = {
  getFileInfo: async (userId: string, filename: string): Promise<FileInfoResponse> => {
    try {
      const response = await fetch('https://backend-aec-experimental.onrender.com/note/get_file_info', {
        method: 'POST',
        headers: {
  ...
  saveFileContent: async (userId: string, filename: string, content: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const response = await fetch('https://backend-aec-experimental.onrender.com/note/save_file_content', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  getConceptExplanations: async (userId: string, filename: string): Promise<ExplanationsResponse> => {
    try {
      const response = await fetch('https://backend-aec-experimental.onrender.com/note/get_explanation_per_concept', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
